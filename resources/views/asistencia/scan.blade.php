        ...
        <div class="w-full sm:max-w-md mt-6 px-8 py-8 bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl overflow-hidden sm:rounded-2xl relative">
            ...
            <div class="mb-6 text-center">
                ...
                <p class="text-xs text-gray-400 mt-1">
                    Introduce el PIN de 7 dígitos para confirmar el escaneo del QR
                </p>
            </div>

            <!-- Session Status / Errors -->
            @if (session('warning'))
                <div class="mb-4 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg text-yellow-400 text-sm font-medium">
                    {{ session('warning') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('asistencia.verify-pin') }}" class="space-y-6">
                @csrf

                <div>
                    <label for="pin" class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">PIN de Acceso</label>
